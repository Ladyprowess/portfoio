...
export default function Media() {
  return <>
      ...
      <div className="mx-auto grid max-w-[1240px] gap-10 lg:grid-cols-[0.9fr_1.1fr] lg:items-center lg:gap-20">
        <div className="max-w-xl">
          ...
          <div className="mt-8 flex flex-col gap-3 sm:flex-row">
            <a href="#portraits" className="inline-flex min-h-12 items-center justify-center rounded-full bg-primary px-7 py-3 text-sm font-bold text-on-primary transition hover:bg-primary-dim">View portraits</a>
            <a href="mailto:hello@ladyprowess.com?subject=Media%20enquiry" className="inline-flex min-h-12 items-center justify-center rounded-full border border-ink-border bg-surface px-7 py-3 text-sm font-bold transition hover:border-primary/40">Media enquiry</a>
          </div>
        </div>
        <div className="relative mx-auto w-full max-w-[540px] overflow-hidden rounded-[2rem] bg-surface-2">
          ...
          <div className="absolute bottom-4 left-4 right-4 rounded-2xl bg-surface/95 px-5 py-4 backdrop-blur"><p className="font-head text-sm font-bold">Ngozi Peace Okafor</p><p className="mt-1 text-sm text-muted">Lady Prowess</p></div>
        </div>
      </div>
    </section>

    <section className="border-b border-ink-border bg-surface px-5 py-14 md:px-8 md:py-20">
      <div className="mx-auto grid max-w-[1240px] gap-8 md:grid-cols-[0.55fr_1.45fr] md:gap-16">
        <div><p className="font-head text-xs font-bold uppercase tracking-[0.18em] text-primary">About</p><h2 className="mt-4 font-display text-2xl font-extrabold">Biography</h2></div>
        <p className="max-w-3xl text-base leading-8 text-muted">Ngozi Peace Okafor is a Digital Product and Content Specialist with experience across technical writing, WordPress design, Web3, fintech, product communication, and content strategy. She has also founded multiple products, where she works with businesses and individuals on digital strategy, content, and technology.</p>
      </div>
    </section>
    ...
    <section className="px-5 pb-20 md:px-8 md:pb-28">
      <div className="mx-auto grid max-w-[1240px] gap-8 rounded-[2rem] bg-panel p-8 text-on-panel md:grid-cols-[1fr_auto] md:items-center md:p-12">
        <div><p className="font-head text-xs font-bold uppercase tracking-[0.18em] text-accent-invert">Need something else?</p><h2 className="mt-4 max-w-2xl font-display text-2xl font-extrabold leading-tight md:text-3xl">For interviews, speaking requests, or a different image format, send a media enquiry.</h2></div>
        <a href="mailto:hello@ladyprowess.com?subject=Media%20enquiry" className="inline-flex min-h-12 items-center justify-center rounded-full bg-on-panel px-7 py-3 text-sm font-bold text-panel">hello@ladyprowess.com</a>
      </div>
    </section>
  </>
}
