...
export default function BlogArchive({ posts }: { posts: BlogArchivePost[] }) {
  ...
  return <>
    <div className="mb-10 space-y-4">
      <div className="flex flex-wrap gap-2" aria-label="Blog categories">{categories.map(item => <button key={item} type="button" onClick={() => { setCategory(item); setPage(1) }} aria-pressed={category === item} className={`rounded-full border px-4 py-2 text-sm font-semibold transition ${category === item ? 'border-primary bg-primary text-white' : 'border-ink-border bg-white text-muted hover:border-primary/40 hover:text-parchment'}`}>{item}</button>)}</div>
      <label className="relative block rounded-2xl border border-ink-border bg-white p-3"><span className="sr-only">Search blog posts</span><svg aria-hidden viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" className="absolute left-7 top-1/2 h-5 w-5 -translate-y-1/2 text-muted"><circle cx="11" cy="11" r="7" /><path d="m20 20-4-4" /></svg><input value={query} onChange={event => { setQuery(event.target.value); setPage(1) }} placeholder={`Search ${category === 'All' ? 'all articles' : category}`} className="w-full rounded-xl bg-bg py-3 pl-12 pr-4 text-sm outline-none focus:ring-2 focus:ring-primary/25" /></label>
    </div>
