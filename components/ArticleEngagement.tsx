...
export function ArticleActions() {
 ...
 return <div data-article-actions hidden={hidden} className="fixed inset-x-3 bottom-[calc(1rem+env(safe-area-inset-bottom))] z-40 mx-auto w-fit max-w-[calc(100%-1.5rem)] rounded-3xl border border-ink-border bg-bg/95 p-2 shadow-lg backdrop-blur-md" onKeyDown={event => { if (event.key === 'Escape') { setOpen(false); document.getElementById(`${id}-button`)?.focus() } }}>
  <div className="flex flex-wrap justify-center gap-1 sm:gap-2" aria-label="Article responses">
   <button type="button" aria-pressed={data.liked} aria-label={data.liked ? 'Unlike this article' : 'Like this article'} disabled={!ready || busy} onClick={toggle} className={`${control} ${data.liked ? 'border-primary text-primary' : 'text-muted'}`}><svg width="18" height="18" viewBox="0 0 24 24" fill={data.liked ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth="1.7" aria-hidden="true"><path d="M20.8 4.6a5.5 5.5 0 0 0-7.8 0L12 5.7l-1.1-1.1a5.5 5.5 0 0 0-7.8 7.8L12 21l8.8-8.6a5.5 5.5 0 0 0 0-7.8Z" /></svg>{data.liked ? 'Liked' : 'Like'}{ready && <span>{data.likes}</span>}</button>
   <a href="#comments" className={control}>Comments {ready && data.commentCount}</a>
   <button id={`${id}-button`} type="button" aria-expanded={open} aria-controls={id} onClick={() => setOpen(!open)} className={control}>Share ↗</button>
  </div>
  {open && <div id={id} className="absolute inset-x-0 bottom-full mb-3 max-h-[50svh] overflow-y-auto rounded-2xl border border-ink-border bg-surface p-4 shadow-lg"><p className="mb-3 text-sm font-semibold">Share this article</p><div className="flex flex-wrap gap-2">{links.map(([name, href]) => <a key={name} href={href} target="_blank" rel="noopener noreferrer" className={control}>{name}</a>)}<button type="button" onClick={copy} className={control}>Copy link</button>{native && <button type="button" onClick={share} className={control}>More…</button>}</div></div>}
