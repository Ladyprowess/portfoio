import { igboHasText, igboEditorHtml, storeIgboHtml } from './igbo-html'
...
export function hasIgboTranslation(post: IgboTranslation) {
 return post.igbo_approved === true && Boolean(post.title_ig?.trim() && post.excerpt_ig?.trim() && (post.body_ig && igboHasText(post.body_ig)))
}
export function translationRecord(payload: Record<string, unknown>) {
 ...
 if (igbo_approved && (!title_ig || !excerpt_ig || !igboHasText(body_ig))) throw new Error('Complete the Igbo title, summary, and article before approving the translation.')
 return { title_ig, excerpt_ig, body_ig: body_ig.startsWith('<!--igbo-rich-text-->') ? storeIgboHtml(igboEditorHtml(body_ig)) : body_ig, igbo_approved }
}
