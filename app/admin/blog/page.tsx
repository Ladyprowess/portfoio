...
function EditorIcon({ name }: { name: IconName }) {
  ...
  if (name === "quote")
    return (
      <svg {...common}>
        <path d="M9.5 11H5.8A1.8 1.8 0 0 0 4 12.8v3.4A1.8 1.8 0 0 0 5.8 18h1.9a1.8 1.8 0 0 0 1.8-1.8V10A4 4 0 0 0 5.5 6M20 11h-3.7a1.8 1.8 0 0 0-1.8 1.8v3.4a1.8 1.8 0 0 0 1.8 1.8h1.9a1.8 1.8 0 0 0 1.8-1.8V10A4 4 0 0 0 16 6" />
      </svg>
    );
  if (name === "code")
    return (
  ...
  if (name === "checklist")
    return (
      <svg {...common}>
        ...
        <path d="m4.5 6.5 1.2 1.2L8 5.2M11 6.5h10" />
...
export default function BlogCmsPage() {
  ...
  const [titleIg, setTitleIg] = useState("");
  const [excerptIg, setExcerptIg] = useState("");
  const [bodyIg, setBodyIg] = useState("");
  const [igboApproved, setIgboApproved] = useState(false);
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      fetch("/api/blog-posts", {
        ...
        body: JSON.stringify({ action: "list", password: saved }),
      })
        .then(async (response) => ({
          response,
        ...
        .then(({ response, data }) =>
          response.ok
            ? setPosts(data.posts || [])
            : setStatus(data.error || "Open the admin dashboard first."),
        )
  ...
  function addTable() {
    ...
    const columns = Math.min(
      Math.max(Number.parseInt(columnAnswer, 10) || 3, 2),
      8,
    );
    const headingCells = Array.from(
      { length: columns },
      (_, index) => `<th scope="col">Heading ${index + 1}</th>`,
    ).join("");
    const bodyRows = Array.from(
      { length: rows - 1 },
      () =>
        `<tr>${Array.from({ length: columns }, () => "<td>Text</td>").join("")}</tr>`,
    ).join("");
  ...
  function addChecklist() {
    ...
    if (selection.isCollapsed) {
      setStatus(
        "Highlight the lines you want to turn into a checklist, then select the checklist icon.",
      );
      return;
    }
    ...
    const items = lines
      .map(
        (line) =>
          `<li><label><input type="checkbox" /><span>${escapeText(line)}</span></label></li>`,
      )
      .join("");
    document.execCommand(
      "insertHTML",
      false,
      `<ul class="blog-checklist">${items}</ul><p><br></p>`,
    );
    setStatus(
  ...
  function edit(post: CmsPost) {
    setId(post.id);
    setTitle(post.title);
    setTitleIg(post.title_ig || "");
    setExcerptIg(post.excerpt_ig || "");
    setBodyIg(post.body_ig || "");
    setIgboApproved(post.igbo_approved === true);
    setSlug(post.slug);
  ...
  function clearForm() {
    setId("");
    setTitle("");
    setTitleIg(""); setExcerptIg(""); setBodyIg(""); setIgboApproved(false);
    setSlug("");
    setExcerpt("");
  ...
  async function sendTest() {
    ...
    try {
      ...
      await sendNewsletter("test", undefined, compactContent);
  ...
  async function save(postStatus: PostState) {
    ...
    try {
      ...
      const data = await request({
        ...
        titleIg, excerptIg, bodyIg, igboApproved,
        ...
        <section className="rounded-3xl border border-ink-border bg-surface p-5 md:p-7">
          <div className="flex items-start justify-between gap-4">
            ...
            {id && (
              <button
                onClick={clearForm}
                className="rounded-full border border-ink-border px-4 py-2 text-xs font-semibold"
              >
                New post
              </button>
            )}
          ...
          <div className="mt-7 grid gap-4 md:grid-cols-2">
            ...
            <label className="text-sm font-semibold md:col-span-2">
              ...
              <textarea
                ...
                placeholder="A short introduction shown on the blog page"
              />
            </label>
            <label className="text-sm font-semibold">
              Category
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            <button
              ...
              className="rounded-full border border-ink-border px-4 py-2 text-xs font-semibold hover:border-primary"
            >
              Upload cover image
            </button>
            {coverImage && (
          ...
          <div className="mt-7 rounded-2xl border border-ink-border bg-surface-2 p-5">
            ...
            <div className="mt-2 rounded-2xl border border-ink-border focus-within:border-primary">
              <div className="sticky top-0 z-30 flex flex-wrap items-center gap-1 rounded-t-2xl border-b border-ink-border bg-surface-2/95 p-2 shadow-[0_8px_24px_rgba(18,18,18,0.07)] backdrop-blur-xl">
                ...
                <ToolbarButton label="Italic" onClick={() => format("italic")}>
                  <span className="font-serif text-base font-bold italic">
                    I
                  </span>
                </ToolbarButton>
                <ToolbarButton
                ...
                >
                  <EditorIcon name="left" />
                </ToolbarButton>
                <ToolbarButton
                  label="Align centre"
                  onClick={() => format("justifyCenter")}
                >
                  <EditorIcon name="centre" />
                </ToolbarButton>
                <ToolbarButton
                  label="Align right"
                  onClick={() => format("justifyRight")}
                >
                  <EditorIcon name="right" />
                </ToolbarButton>
                <ToolbarButton
                  label="Justify text"
                  onClick={() => format("justifyFull")}
                >
                  <EditorIcon name="justify" />
                </ToolbarButton>
                <span className="mx-1 h-6 w-px bg-ink-border" aria-hidden />
                <ToolbarButton
                  label="Bullet list"
                ...
                >
                  <EditorIcon name="bullets" />
                </ToolbarButton>
                <ToolbarButton
                  label="Numbered list"
                  onClick={() => format("insertOrderedList")}
                >
                  <EditorIcon name="numbers" />
                </ToolbarButton>
                <ToolbarButton label="Checklist" onClick={addChecklist}>
              ...
              <div className="mx-auto max-w-4xl rounded-3xl bg-surface p-6 shadow-2xl md:p-10">
                <div className="flex items-center justify-between gap-4 border-b border-ink-border pb-5">
                  ...
                  <button
                    ...
                    className="rounded-full border border-ink-border px-4 py-2 text-sm font-semibold"
                  >
                    Close
                  </button>
                </div>
          ...
          <div className="rounded-3xl border border-ink-border bg-surface p-5">
            ...
            <div className="mt-4 space-y-2">
              <input
                ...
                className="w-full rounded-xl border border-ink-border bg-bg px-3 py-2.5 text-sm outline-none focus:border-primary"
              />
              <select
                value={postFilter}
                onChange={(event) => {
            ...
            {matchingPosts.length > POSTS_PER_PAGE && (
              <div className="mt-5 flex items-center justify-between border-t border-ink-border pt-4">
                <button
                  ...
                  className="rounded-full border border-ink-border px-3 py-1.5 text-xs font-semibold disabled:opacity-35"
                >
                  Previous
                </button>
                <span className="text-xs text-muted">
                  {safePostPage} of {totalPostPages}
                </span>
                <button
                  disabled={safePostPage === totalPostPages}
                  onClick={() =>
                    setPostPage((page) => Math.min(totalPostPages, page + 1))
                  }
                  className="rounded-full border border-ink-border px-3 py-1.5 text-xs font-semibold disabled:opacity-35"
                >
                  Next
