...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const posts = [...cmsPosts.map(post => ({ slug: post.slug, category: post.category, modified: post.updated_at || post.published_at })), ...blogPosts.map(post => ({ slug: post.slug, category: post.category, modified: null }))]
  ...
  return [
    ...
    ...categories.map(category => ({ url: `${base}/blog/category/${categorySlug(category)}`, lastModified: new Date(), changeFrequency: 'weekly' as const, priority: 0.7 })),
